...
class SvgUri extends Component{
  ...
  createSVGElement(node, childs){
    ...
    case 'svg':
      ...
      return <Svg fill={this.props.fill} key={i} {...componentAtts}>{childs}</Svg>;
    case 'g':
      componentAtts = this.obtainComponentAtts(node, G_ATTS);
      return <G fill={this.props.fill} key={i} {...componentAtts}>{childs}</G>;
    case 'path':
    ...
    case 'circle':
      ...
      return <Circle fill={this.props.fill} key={i} {...componentAtts}>{childs}</Circle>;
    case 'rect':
      componentAtts = this.obtainComponentAtts(node, RECT_ATTS);
      return <Rect fill={this.props.fill} key={i} {...componentAtts}>{childs}</Rect>;
    case 'line':
      componentAtts = this.obtainComponentAtts(node, LINE_ATTS);
      return <Line fill={this.props.fill} key={i} {...componentAtts}>{childs}</Line>;
    case 'defs':
      return <Defs fill={this.props.fill} key={i}>{childs}</Defs>;
    case 'linearGradient':
      componentAtts = this.obtainComponentAtts(node, LINEARG_ATTS);
      return <LinearGradient fill={this.props.fill} key={i} {...componentAtts}>{childs}</LinearGradient>;
    case 'radialGradient':
      componentAtts = this.obtainComponentAtts(node, RADIALG_ATTS);
      return <RadialGradient fill={this.props.fill} key={i} {...componentAtts}>{childs}</RadialGradient>;
    case 'stop':
      componentAtts = this.obtainComponentAtts(node, STOP_ATTS);
      return <Stop fill={this.props.fill} key={i} {...componentAtts}>{childs}</Stop>;
    case 'ellipse':
      componentAtts = this.obtainComponentAtts(node, ELLIPSE_ATTS);
      return <Ellipse fill={this.props.fill} key={i} {...componentAtts}>{childs}</Ellipse>;
    case 'polygon':
      componentAtts = this.obtainComponentAtts(node, POLYGON_ATTS);
      return <Polygon fill={this.props.fill} key={i} {...componentAtts}>{childs}</Polygon>;
    case 'polyline':
      componentAtts = this.obtainComponentAtts(node, POLYLINE_ATTS);
      return <Polyline fill={this.props.fill} key={i} {...componentAtts}>{childs}</Polyline>;
    case 'text':
      componentAtts = this.obtainComponentAtts(node, TEXT_ATTS);
      if (componentAtts.y) {
        componentAtts.y = fixYPosition(componentAtts.y, node)
      }
      return <Text fill={this.props.fill} key={i} {...componentAtts}>{childs}</Text>;
    case 'tspan':
      componentAtts = this.obtainComponentAtts(node, TEXT_ATTS);
      if (componentAtts.y) {
        componentAtts.y = fixYPosition(componentAtts.y, node)
      }
      return <TSpan fill={this.props.fill} key={i} {...componentAtts}>{childs}</TSpan>;
    default:
      return null;
    }
  }
